...
A real-time cosmic radar that shows you what is in the sky above **any location on Earth** - the ISS, planets, bright stars, the Moon, and the Sun - computed with astronomical precision and rendered through three immersive visualizations.

**[Live Demo](https://zenith-smfawaz24.vercel.app)** · **[GitHub](https://github.com/SMFawaz24/zenith)** · Built for the Astral Web Innovate Hackathon

---

## What It Does

Pick any location on Earth and instantly see what is in the sky above it right now.

Type a city name, click on the globe, or share your GPS coordinates - ZENITH computes the real-time position of **30+ celestial objects** using astronomical algorithms and displays them across three interactive views: a 3D rotating globe, a polar sky map, and an animated radar sweep.
...
- **Sky Map** - Stereographic polar projection showing the dome of the sky above you (zenith at center, horizon at edge)
...
- **ISS Live Tracking** - Position fetched every 5 seconds from NASA's API, with orbit trail drawn on the globe
- **ISS Telemetry** - Live speed (km/s), orbital altitude, and lat/lon coordinates
- **ISS Pass Predictions** - Next 5 upcoming passes with time, duration, and max elevation for your location
- **ISS Ground Track** - Mini SVG world map showing the orbital path and current position
...
- **Globe click** - Click anywhere on the 3D globe to set the observer position
- **Manual coordinates** - Enter `lat, lon` directly (e.g., `40.7, -74.0`)

### UI/UX

- **Moon phase visualization** - Phase name, illumination percentage, and rendered phase graphic
- **Object detail cards** - Click any object for altitude, azimuth, distance, and magnitude
- **Filter toggles** - Show/hide ISS, Satellites, Planets, Stars, Moon, Sun, Constellations, and Orbits
...
Returns real-time ISS latitude, longitude, altitude (km), and velocity (km/h). Polled every **5 seconds**. On failure, falls back to orbital simulation using:
...
All celestial positions are computed **client-side** using formulas from Jean Meeus' *Astronomical Algorithms* (the standard reference for positional astronomy). No external APIs are required.
...
Greenwich Mean Sidereal Time indicates which stars are overhead at Greenwich; adding the observer's longitude gives Local Sidereal Time.

### Coordinate Conversion (Equatorial to Horizontal)

Converts celestial coordinates (RA/Dec) to observable values (altitude above horizon, compass direction):
...
Simplified mean orbital elements propagated from the J2000 epoch:
...
16 of the brightest stars from the FK5 J2000 catalog with fixed RA/Dec coordinates. Stellar proper motion is negligible over human timescales.

---

## Design Philosophy

**Direction: Obsidian and Starlight** - inspired by Brian Eno's Cosmos app, Apple Vision Pro spatial UI, and Monocle magazine.

| Principle | Implementation |
|---|---|
| One strong visual idea | The globe and sky map are the interface, not a background behind panels |
| Restraint | Pure black `#000000`, white text, single desaturated accent `#a8c4d4` |
| Negative space | No permanently visible sidebars; elements appear contextually |
...
| 60fps | All animations use `requestAnimationFrame` with no DOM thrashing |
...
Drag and drop the folder at `app.netlify.com/drop`, or connect your GitHub repo at `netlify.com`.
...
Given more time, the following would be the next additions:

- **WebGL globe** - Replace Canvas 2D with Three.js for a photorealistic Earth with cloud layers and a day/night terminator
- **TLE parsing** - Use real Two-Line Element sets for accurate satellite tracking instead of simulated orbits
- **Notification system** - Push alerts when the ISS is about to pass over a given location
- **Augmented reality** - Use the device orientation API to overlay the sky map on a phone camera feed
- **Deep sky objects** - Full Messier catalog (galaxies, nebulae, star clusters)
- **Historical mode** - Time scrubbing to view the sky at any date and time
- **Multi-language support** - i18n support for the interface
- **PWA** - Offline capability with a service worker for cached star and planet data
